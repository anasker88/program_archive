...
let rec generize tyenv ty = match ty with
    ...
    | TyList a -> let (tvli,ty)=generize tyenv a in (tvli,TyList(ty))
    | TyPair (a,b)-> let (tvli1,ty1)=generize tyenv a in
                    let (tvli2,ty2)=generize tyenv b in
                    ((connect_as_set tvli1 tvli2),TyPair (ty1,ty2))
...
let rec get_type_vars ty = match ty with
    ...
    | TyList a -> get_type_vars a
    | TyPair (a,b) -> connect_as_set (get_type_vars a) (get_type_vars b)
    | TyVar a -> [a]
...
let rec infer_pattern (tyenv:type_env) p=match p with
| PInt _-> (TyInt,[],tyenv)
| PBool _-> (TyBool,[],tyenv)
| PVar x-> let a=new_tyvar () in
    (TyVar a,[],extend x ([],(TyVar a)) tyenv)
|PNil -> let a=new_tyvar () in
    (TyList (TyVar a),[],tyenv)
|PCons (p,pli)->let (ty1,con1,env1)= infer_pattern tyenv p in 
    let (ty2,con2,env2)= infer_pattern tyenv pli in
    (ty2,(ty2,TyList ty1)::con1@con2,connect_as_set env1 env2)
|PPair (p1,p2) ->let (ty1,con1,env1)= infer_pattern tyenv p1 in 
    let (ty2,con2,env2)= infer_pattern tyenv p2 in
    (TyPair(ty1,ty2),con1@con2,connect_as_set env1 env2)
...
let rec infer_expr (tyenv:type_env) e = match e with
  ...
  | ECons (e1,e2)-> 
      let (ty1,con1)= infer_expr tyenv e1 in 
      let (ty2,con2)= infer_expr tyenv e2 in
      (ty2, (TyList ty1,ty2)::con1@con2)
  | EMatch (e1,patli) -> 
    let (ty1,con1)= infer_expr tyenv e1 in 
    (match patli with
  | [] -> raise TypeErr
  | (pat,e2)::[]-> let (typ,conp,envp)= infer_pattern tyenv pat in
        let (ty2,con2)=infer_expr envp e2 in
        let a = new_tyvar () in
        (TyVar a,(ty1,typ)::(TyVar a,ty2)::con1@conp@con2)
  | (pat,e2)::rest -> let (typ,conp,envp)= infer_pattern tyenv pat in
  let (ty2,con2)=infer_expr envp e2 in
  let (curty,curcon)=infer_expr tyenv (EMatch (e1,rest)) in
  (curty,(ty1,typ)::(curty,ty2)::conp@con2@curcon)
                   )
