...
| TyList (ty) -> print_type ty;print_string("list ")
| TyPair (ty1,ty2) -> print_type ty1;print_string(" * ");print_type ty2
| TyVar num -> print_string("a");print_int(num);print_string(" ")
...
let rec ty_subst (sbt:subst) t=
  ...
  | TyList a -> TyList (ty_subst sbt a)
  | TyPair (a,b) -> TyPair (ty_subst sbt a,ty_subst sbt b)
...
let rec exist_in_expr_type ty tyvar =match ty with
    ...
    |TyList a -> exist_in_expr_type a tyvar
    |TyPair (a,b) -> (exist_in_expr_type a tyvar) || (exist_in_expr_type b tyvar)
    |TyVar a -> a=tyvar
...
| (ty1,ty2) :: rest -> 
  ...
  | (TyPair (s1,t1),TyPair (s2,t2)) -> unify ((s1,s2)::(t1,t2)::rest)
